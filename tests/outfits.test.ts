...
import { getOutfitIds, hasMultipleOutfits, normalizeOutfitId } from "../src/shared/outfit";
...
describe("generated outfit catalog", () => {
  for (const character of characters) {
    it(`keeps ${character.id} indices and translations aligned with characters.json`, () => {
      ...
      expect(getOutfitIds(id)).toEqual(outfits.map((_, index) => index));
      expect(hasMultipleOutfits(id)).toBe(outfits.length > 1);
      expect(en.characters.outfits[id]).toEqual(Object.fromEntries(outfits.map((outfit, index) => [index, outfit.en])));
      expect(zh.characters.outfits[id]).toEqual(Object.fromEntries(outfits.map((outfit, index) => [index, outfit.zh])));
    });
  }

  it("defaults missing metadata after an outfit catalog expansion", () => {
    expect(hasMultipleOutfits("Belle")).toBe(true);
