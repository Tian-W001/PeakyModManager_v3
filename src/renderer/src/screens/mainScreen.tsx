...
const MainScreen: React.FC = () => {
  ...
  const [toggleCharacterbarOpen, shouldCharacterbarMount, shouldCharacterbarTransition] = useMountTransition(150);
  useEffect(() => {
        ...
        <div className="flex min-h-0 flex-1 flex-row" id="content-area">
          ...
          <div className="relative flex h-full flex-1 flex-col overflow-hidden" id="card-grid-area">
            {shouldCharacterbarMount && (
              <CharacterBar
                className={`absolute top-0 left-0 z-10 mt-4 h-14 w-full max-w-[80%] transition-transform duration-150 ease-out ${shouldCharacterbarTransition ? "translate-y-0" : "-translate-y-[calc(100%+16px)]"}`}
              />
            )}
            <ModCardGrid
